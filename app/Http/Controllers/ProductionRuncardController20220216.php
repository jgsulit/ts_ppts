...
class ProductionRuncardController20220216 extends Controller
{
    //
        ...
        return DataTables::of($prod_runcards)
            ->addColumn('raw_action', function($prod_runcard){
                ...
                $result='<button style="width:30px;" runcard-id="' . $prod_runcard->id . '" title="Open details" class="btnOpenRuncardDetails btn btn-info btn-sm py-0"><i class="fa fa-info-circle fa-sm"></i></button>';
                 ...
                 // $result .=' <button style="width:30px;" lot-no="' . $prod_runcard->runcard_no . '" po-no="'.$prod_runcard->po_no.'" lot-qty="'.$col_prod.'" title="Print Lot Number" class="btn-print-lot-no btn btn-success btn-sm py-0" '.$disabled.'><i class="fa fa-print fa-sm"></i></button>';
                            ...
                            if( $request->txt_prod_runcard_station_id_query ){
                                ...
                                // if($validator->passes()){
                                    ...
                                    try { 
                                        ...
                                        $prd_runcard = ProductionRuncard::where('id', $stt_id[0]->production_runcard_id)->get();
                                        ...
                                        $ttl_po_qty = 0;
                                        $all_ProductionRuncard = ProductionRuncard::where('po_no', $prd_runcard[0]->po_no)->get();
                                        for ($i=0; $i < count($all_ProductionRuncard); $i++) { 
                                            $prd_run_stt = ProductionRuncardStation::where('production_runcard_id', $all_ProductionRuncard[$i]->id)->where('id', '!=', $stt_id[0]->id)->where('status', 1)->get();
                            ...
                            // if( $request->txt_prod_runcard_station_id_query ){
                                ...
                                // if($validator->passes()){
                                    ...
                                    try {
                                        ...
                                        // return $final_runcard_no;
                                        $prd_runcard = ProductionRuncard::where('id', $request->txt_prod_runcard_id_query)->get();
                                        $dvc = Device::where('name', $prd_runcard[0]->device_name)->get();
                            ...
                            // if($validator->passes()){
                                ...
                                if(count($selected_prod_runcard) > 0){
                                    ...
                                    try {
                                        ...
                                        // return $final_runcard_no;
                                        $prd_runcard = ProductionRuncard::where('id', $selected_prod_runcard[$outer_index])->get();
                                        $dvc = Device::where('name', $prd_runcard[0]->device_name)->get();
